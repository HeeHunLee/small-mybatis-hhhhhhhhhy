package com.hhhhhhhhhy.mybatis.dataSource.pooled;

import com.hhhhhhhhhy.mybatis.dataSource.unpooled.UnpooledDataSource;
...
public class PooledDataSource implements DataSource {
    ...
    private PooledConnection popConnection(String username, String password) throws SQLException {
        ...
        // while 死循环，只有获取到链接抛出异常才会退出循环
        while (conn == null) {
            // 获取链接使用 synchronized 重量级锁，因为所有线程在资源竞争的情况下都需要加锁
            synchronized (state) {
                // 如果有空闲连接，返回第一个
                if (!state.idleConnections.isEmpty()) {
                    System.out.println("空闲连接数有：" + state.idleConnections.size());
                ...
                // 如果无空闲连接，创建新连接
                    ...
                    // 活跃连接数已满
                        ...
                        // 获取活跃链接列表的第一个，也就是存活最久的链接
                        PooledConnection oldestActiveConnection = state.activeConnections.get(0);
                        long longestCheckoutTime = oldestActiveConnection.getCheckoutTime();
                        // 如果 checkout 的时间过长，则将这个链接标记为过期
    ...
    /**
     * 连接 JDBC 中间件数据源，并关闭所有链接（活跃 and 空闲）
     * @param driver
     */
